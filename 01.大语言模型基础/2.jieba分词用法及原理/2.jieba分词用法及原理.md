...
1. **精确模式**\*\*：**就是把一段文本精确地切分成若干个中文单词，若干个中文单词之间经过组合，就精确地还原为之前的文本。其中**不存在冗余单词 \*\*。
   text = "研究生命起源"
  输出：['研究', '生命', '起源']
3. **全模式**\*\*：**将一段文本中所有可能的词语都扫描出来，可能有一段文本它可以切分成不同的模式，或者有不同的角度来切分变成不同的词语，在全模式下，Jieba库会将各种不同的组合都挖掘出来。分词后的信息再组合起来**会有冗余，不再是原来的文本 \*\*。
   text = "研究生命起源"
   输出：['研究', '研究生', '生命', '起源']
5. **搜索引擎模式****：** 在精确模式基础上，对发现的那些长的词语，我们会对它再次切分，进而适合搜索引擎对短词语的索引和搜索。**也有冗余**。
   text = "中华人民共和国"
   输出：['中华', '人民', '共和国', '中华人民共和国']
7. **paddle模式**：利用PaddlePaddle深度学习框架，训练序列标注（双向GRU）网络模型实现分词。同时支持词性标注。paddle模式使用需安装paddlepaddle-tiny，`pip install paddlepaddle-tiny==1.6.1`。目前paddle模式支持jieba v0.40及以上版本。jieba v0.40以下版本，请升级jieba，`pip install jieba --upgrade` 。
...
- 采用了**动态规划查找最大概率(词频)路径, 找出基于词频的最大切分组合**
- 对于**未登录词（词典中没有的词）**，**采用了基于汉字成词能力的 HMM 模型，使用了 Viterbi 算法**
